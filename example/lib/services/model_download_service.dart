...
class ModelDownloadService {
  ...
  Future<bool> initialize() async {
    ...
    if (status != PermissionStatus.granted) {
      debugPrint(
        '[ModelDownload] Storage permission not granted, requesting manageExternalStorage...',
      );
      status = await Permission.manageExternalStorage.request();
      debugPrint(
        '[ModelDownload] ManageExternalStorage permission status: $status',
      );
    }

    if (status != PermissionStatus.granted) {
      debugPrint(
        '[ModelDownload] ⚠ Warning: Storage permission not granted: $status',
      );
    } else {
    ...
    debugPrint(
      '[ModelDownload] Checking for existing model at: ${modelFile.path}',
    );

    if (await modelFile.exists()) {
      final fileSize = await modelFile.length();
      debugPrint(
        '[ModelDownload] File found! Size: ${(fileSize / 1024 / 1024).toStringAsFixed(2)} MB',
      );

      // Validate file size (model should be at least 10 MB)
      if (fileSize < 10 * 1024 * 1024) {
        debugPrint(
          '[ModelDownload] ⚠ WARNING: File is too small ($fileSize bytes)',
        );
        debugPrint(
          '[ModelDownload] ⚠ This is likely an empty or corrupted file.',
        );
        debugPrint(
          '[ModelDownload] ⚠ Please use "Load from Local" to select your actual model file.',
        );
  ...
  Future<String?> pickLocalModel() async {
    ...
    try {
      debugPrint(
        '[ModelDownload] Opening file picker dialog with FileType.any',
      );
      ...
      debugPrint(
        '[ModelDownload] File picker result: ${result != null ? "Got result" : "null"}',
      );
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (await file.exists()) {
          ...
          debugPrint(
            '[ModelDownload] ✓ File exists! Size: ${fileSizeMB.toStringAsFixed(2)} MB ($fileSize bytes)',
          );

          // Validate file size
          if (fileSize < 10 * 1024 * 1024) {
            debugPrint(
              '[ModelDownload] ✗✗✗ ERROR: File is too small (${fileSizeMB.toStringAsFixed(2)} MB)',
            );
            debugPrint(
              '[ModelDownload] ✗ A valid GGUF model should be at least 100+ MB',
            );
            debugPrint(
              '[ModelDownload] ✗ This file is likely empty or corrupted',
            );
            return null;
          }
          ...
          if (!filePath.toLowerCase().endsWith('.gguf')) {
            debugPrint(
              '[ModelDownload] ⚠ Warning: Selected file does not have .gguf extension: $filePath',
            );
            debugPrint(
              '[ModelDownload] ⚠ Continuing anyway - make sure this is a valid GGUF model',
            );
          } else {
          ...
          _modelPath = filePath;
          debugPrint(
            '[ModelDownload] ✓ Model path set successfully: $filePath',
          );
          return filePath;
        } else {
          debugPrint(
            '[ModelDownload] ✗ Error: File does not exist at path: $filePath',
          );
        }
      } else {
