...
class ChatService {
  ...
  Future<void> loadModel({
    ...
    try {
      ...
      // Detect GPU capabilities before loading
      onStatus('Detecting GPU...');
      int gpuLayers = 0;
      try {
        final gpu = await _llama!.detectGpu();
        gpuLayers = gpu.recommendedGpuLayers;
        debugPrint('[ChatService] GPU detection result:');
        debugPrint('[ChatService]   - vulkanSupported: ${gpu.vulkanSupported}');
        debugPrint('[ChatService]   - gpuName: ${gpu.gpuName}');
        debugPrint('[ChatService]   - freeRam: ${(gpu.freeRamBytes / 1024 / 1024).toStringAsFixed(0)} MB');
        debugPrint('[ChatService]   - recommendedGpuLayers: ${gpu.recommendedGpuLayers}');
      } catch (e) {
        debugPrint('[ChatService] GPU detection failed, using CPU: $e');
      }
      ...
      debugPrint('[ChatService]   - gpuLayers: $gpuLayers');

      onStatus(gpuLayers > 0 ? 'Loading model (GPU $gpuLayers layers)...' : 'Loading model (CPU)...');
